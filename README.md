...
3. **結果を確認**
   ...
   - TOP3カードはスポット写真が登録されている場合、カード上部にヒーロー写真として表示される
     （2026-07-26追加。その他一覧・お気に入り一覧は従来通りホバー/長押しプレビューのみ）
...
6. **釣果投稿**
   ...
   - 自分の投稿には編集ボタン（鉛筆アイコン）が表示され、本文・釣れた魚種・写真を後から変更できる
     （2026-07-26追加。投稿先スポットは変更不可。編集すると投稿に「（編集済み）」と表示される）
...
13. **AI分析・新スポット探索・釣具店検索のコスト保護（2026-07-26追加）**
    ...
    - 上限に達した状態でボタンを押すと、429エラーとともに「本日の利用回数（n件）に達しました。
      明日またお試しください。」という案内が表示される（ボタンの見た目は他の失敗時と同じ赤い枠）
    - 上限値は `admin_trigger.py` 経由の2つは `template.yaml` の `RATE_LIMIT_DAILY` 環境変数、
      釣具店検索は `tackle_shops.py` の `DAILY_SEARCH_LIMIT` で調整できる
